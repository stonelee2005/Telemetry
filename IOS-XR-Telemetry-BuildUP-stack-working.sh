...
mkdir -p ~/IOSXR-Telemetry-Collection-Stack/analytics/influxdb && cd ~/IOSXR-Telemetry-Collection-Stack/analytics/influxdb/
...
mkdir -p ~/IOSXR-Telemetry-Collection-Stack/analytics/telegraf && cd ~/IOSXR-Telemetry-Collection-Stack/analytics/telegraf/
## Check the MD5 hash from the download
...
cp ~/IOSXR-Telemetry-Collection-Stack/SNMP-Telegraf/SNMP-MIBS-Telegraf ~/IOSXR-Telemetry-Collection-Stack/analytics/telegraf/
...
mkdir -p ~/IOSXR-Telemetry-Collection-Stack/analytics/kapacitor && cd ~/IOSXR-Telemetry-Collection-Stack/analytics/kapacitor/
## Check the MD5 hash from the download
...
cp ~/IOSXR-Telemetry-Collection-Stack/Kapacitor/* ~/IOSXR-Telemetry-Collection-Stack/analytics/kapacitor/
sed -i "s/localhost/$SERVER_IP/" ~/IOSXR-Telemetry-Collection-Stack/analytics/kapacitor/CPU-ALERT-ROUTERS.tick
sed -i "s/localhost/$SERVER_IP/" ~/IOSXR-Telemetry-Collection-Stack/analytics/kapacitor/KAPACITOR-HELPER-CPU.py
...
sed -i "s/TokenID/$TOKEN_MODIFIED/" ~/IOSXR-Telemetry-Collection-Stack/analytics/kapacitor/KAPACITOR-HELPER-CPU.py
sed -i "s/channelname/$SLACK_CHANNEL/" ~/IOSXR-Telemetry-Collection-Stack/analytics/kapacitor/KAPACITOR-HELPER-CPU.py
sed -i "s/uname/$SLACK_USERNAME/" ~/IOSXR-Telemetry-Collection-Stack/analytics/kapacitor/KAPACITOR-HELPER-CPU.py
## Starting Kapacitor
sudo systemctl start kapacitor >/dev/null
## Applying and activating our tick script and helper file
sleep 1
kapacitor define CPU-ALERT-ROUTERS -tick ~/IOSXR-Telemetry-Collection-Stack/analytics/kapacitor/CPU-ALERT-ROUTERS.tick
kapacitor enable CPU-ALERT-ROUTERS
python ~/IOSXR-Telemetry-Collection-Stack/analytics/kapacitor/KAPACITOR-HELPER-CPU.py & > /dev/null
sleep 2
...
mkdir -p ~/IOSXR-Telemetry-Collection-Stack/analytics/prometheus && cd ~/IOSXR-Telemetry-Collection-Stack/analytics/prometheus/
## Check the MD5 hash from the download
...
sed -i "s/localhost/$SERVER_IP/" ~/IOSXR-Telemetry-Collection-Stack/analytics/prometheus/prometheus-1.5.2.linux-amd64/prometheus.yml
echo "  - job_name: 'pipeline'" >> ~/IOSXR-Telemetry-Collection-Stack/analytics/prometheus/prometheus-1.5.2.linux-amd64/prometheus.yml
echo "    static_configs:" >> ~/IOSXR-Telemetry-Collection-Stack/analytics/prometheus/prometheus-1.5.2.linux-amd64/prometheus.yml
echo -e "      - targets: ['$SERVER_IP:8989']" >> ~/IOSXR-Telemetry-Collection-Stack/analytics/prometheus/prometheus-1.5.2.linux-amd64/prometheus.yml
## Start Prometheus
sleep 2
cd ~/IOSXR-Telemetry-Collection-Stack/analytics/prometheus/prometheus-1.5.2.linux-amd64/
...
mkdir -p ~/IOSXR-Telemetry-Collection-Stack/analytics/grafana && cd ~/IOSXR-Telemetry-Collection-Stack/analytics/grafana/
## Check the MD5 hash from the download
...
cd ~/IOSXR-Telemetry-Collection-Stack/analytics

## Check the MD5 hash from the download
MD5_Pipeline=`md5sum -c <<<"a90afc7b39d612d58be4bfa6834e37f0  telemetry-pipeline-final.zip"`
## A basic notification if things went wrong
...
echo -e "\e[1;32m Unpacking the file and making changes in the config file \e[0m";
unzip telemetry-pipeline-final.zip > /dev/null
## Renaming the directory
#mv bigmuddy-network-telemetry-pipeline-master/ pipeline
mv bigmuddy-network-telemetry-pipeline-final/ pipeline
## Installing the metrics.json file for all the sensor paths used in the Stack
cp ~/IOSXR-Telemetry-Collection-Stack/Pipeline/metrics.json ~/IOSXR-Telemetry-Collection-Stack/analytics/pipeline/
## Saving original pipeline.conf file
mv ~/IOSXR-Telemetry-Collection-Stack/analytics/pipeline/pipeline.conf ~/IOSXR-Telemetry-Collection-Stack/analytics/pipeline/pipeline-original-github.conf
## Copying pre-defined pipeline.conf and updating IP address
cp ~/IOSXR-Telemetry-Collection-Stack/Pipeline/pipeline.conf ~/IOSXR-Telemetry-Collection-Stack/analytics/pipeline/
sed -i "s/1.2.3.4/$SERVER_IP/" ~/IOSXR-Telemetry-Collection-Stack/analytics/pipeline/pipeline.conf
cd ~/IOSXR-Telemetry-Collection-Stack/analytics/pipeline/bin && touch dump.txt
## Stating Pipeline from a screen
echo -e "\e[1;32m Starting Pipeline (in a screen) \e[0m";
screen -dm -S Pipeline bash -c 'cd ~/IOSXR-Telemetry-Collection-Stack/analytics/pipeline; sudo ./bin/pipeline -config pipeline.conf -pem ~/.ssh/id_rsa; exec /bin/bash'
